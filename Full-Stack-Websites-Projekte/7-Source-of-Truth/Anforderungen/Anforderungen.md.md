...
<!-- ERWEITERUNG: GEO-TARGETING & SEO-SILOING -->

# Fachliche Anforderungen: Geo-Targeting & SEO-Siloing

## 1. Funktionale Anforderungen (Multi-Location)
- Dynamisches Routing: Das System muss via Next.js Dynamic Routes (app/[service]/[stadt]/page.tsx) performant skalierbar sein, um beliebig viele Städte ohne manuellen Code-Aufwand hinzuzufügen.
- Bidirektionale Verlinkung: Jede generierte Unterseite der Ebene 3 muss automatisiert und ohne Hardcoding Backlinks zum übergeordneten Service-Kategorie-Hub und zum Standort-Verzeichnis aufweisen.
- Crawlability: Generierung einer dynamischen sitemap.xml, die die Hierarchie (Homepage -> Hubs -> Local Pages) für Suchmaschinen-Crawler logisch abbildet.

## 2. Nicht-funktionale Anforderungen & Validierung
- Ladezeit (Core Web Vitals): Da Ebene-3-Seiten oft über Google Ads oder lokales SEO angesteuert werden, müssen diese via Static Site Generation (SSG) vorgerendert und mittels ISR (Incremental Static Regeneration) aktuell gehalten werden.
- Kein Duplicate Content: Der Agent muss bei der Content-Generierung für die lokalen Service-Seiten sicherstellen, dass stadt-spezifische Variablen (z. B. Wahrzeichen, regionale Referenzen) injiziert werden, um textliche Identität zu vermeiden.